	...
	public List<CartTransaction> getList(List<ManagementQuery> queries) {
		...
		try {
			...
			if (queries != null && queries.size() > 0) {
				getListQuery += " WHERE " + queries.get(0).toString(true);
			}
			...
			while (rs.next()) {
				...
				for (int i = 0, j = 1, k = getListResult.length; i < k; i++, j++)
					if (rs.getObject(j) != null)
						transaction.setInfo(getListResult[i], rs.getObject(getListResult[i]).toString());
